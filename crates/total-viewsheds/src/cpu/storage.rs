...
impl PolarSegments {
    ...
    #[expect(clippy::expect_used, reason="We assume everything fits in a u16, we want a panic if it doesn't")]
...
impl Engine for SqliteEngine {
    fn store_segments(&self, tvs_id: u32, segments: PolarSegments) {
        #[expect(clippy::expect_used, reason="we should crash to make sure not to deadlock")]
        self.sender
...
impl Drop for SqliteEngine {
    fn drop(&mut self) {
        ...
        #[expect(clippy::expect_used, reason="we really should crash under any of these conditions")]
        self.worker_handle
            .take()
...
mod test {
    ...
    #[test]
    #[expect(clippy::indexing_slicing, reason="testing, slicing/panicking is okay")]
    fn bitmap_to_angle() {
        {
