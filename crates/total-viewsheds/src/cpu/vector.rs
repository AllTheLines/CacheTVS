...
impl VectorGreater<16> for VectorLos<16> {
    #[inline]
    fn gt(angle: Simd<f32, 16>, prefix: Simd<f32, 16>) -> Mask<i32, 16> {
...
impl PrefixMax for VectorLos<16> {
    #[inline]
    fn prefix_max(highest: f32, angles_in: &[f32], prefix_max: &mut [f32]) {
        use std::arch::x86_64::{
        ...
        let (vector_prefix, _) = prefix_max.as_chunks_mut::<16>();
    ...
    fn accumulate(
        ...
        .for_each(
            |(sum_arr, longest_arr, &angle_arr, &prefix_arr, &distances_arr)| {
                ...
                let acc = Simd::from(*sum_arr)
                    + (mask.select(dist * Simd::splat(TAN_ONE_RAD), Simd::splat(0.0f32)));
